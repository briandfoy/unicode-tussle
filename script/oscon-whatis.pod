...
Because I misplaced the first of these I wrote, I ended up writing them twice.
...
A few use Unicode not just in literals, but in identifiers, too:
    ...
    % tcgrep '((^\h*sub\h+)|[\$\@%&])\p{ASCII}*\P{ASCII}' *
    byte2uni:           $display_char = "\N{SYMBOL FOR SUBSTITUTE FORM TWO}",        # ␦
    hantest:$path = "婴儿服饰";
    hypertest:my @ὑπέρμεγας = ( 
    hypertest:    ὑπέρμεγας           => \@ὑπέρμεγας,
    leo:        my    $ʇndʇno = uʍopəpᴉƨdn($input);
    leo:        say   $ʇndʇno;
    leo:sub uʍopəpᴉƨdn($) {
    leo:    tr [-¯_#&'"“”‘’!¡?¿,.] 
    mismaps:my @ɪsᴏ = map { "iso-$_" } ratsort qw{
    mismaps:my @μsoft = map { "cp$_"} ratsort qw{
    mismaps:my @鯉 = ratsort <koi8-{f,u,r}>;
    mismaps:my @all_tests =  (@μsoft, @ɪsᴏ, @apple, @鯉, @etc);
    mismaps:        dos         => \@μsoft,
    mismaps:        microsoft   => \@μsoft,
    mismaps:        ms          => \@μsoft,
    mismaps:        windows     => \@μsoft,
    mismaps:        win         => \@μsoft,
    mismaps:        posix       => \@ɪsᴏ,
    mismaps:        iso         => \@ɪsᴏ,
    mismaps:        standard    => \@ɪsᴏ,
    mismaps:        std         => \@ɪsᴏ,
    mismaps:        koi         => \@鯉,
...
=head1 Demos of OSCON Unicode scripts

There's absolutely nothing like examples, so here are five sets.
